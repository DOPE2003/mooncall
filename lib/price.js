// lib/price.js
// Unified token info for Mooncall cards + helpers (Dexscreener + Pump.fun)

let getMintSafety = async () => ({});
let extStripPump;
try {
  const mod = require('./solana');                    // optional; if missing we’ll fallback
  if (mod.getMintSafety) getMintSafety = mod.getMintSafety;
  if (mod.stripPump) extStripPump = mod.stripPump;
...
const stripPump = (m) => extStripPump ? extStripPump(m) : String(m || '').replace(/pump$/i, '');
...
async function fromPumpfun(mintRaw) {
  ...
  // Heuristic: show 100% burn only once curve is complete
  ...
  return {
    ...
    // For the “DexS …” line, while on curve Solbix shows ❌
    dexPaid: false,
  };
}
  ...
  try {
    ...
    if (pairs?.length) {
      ...
      const base = {
        ...
        dexPaid: undefined, // neutral when listed
      };
